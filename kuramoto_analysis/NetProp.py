...
def multi_coupling(file_coupling,file_connectivity):
    coupling = np.load(file_coupling)
    connectivity = np.load(file_connectivity)
    num_coupling = coupling.shape[0]
    if type(connectivity)==list:
        H = True
    else:
        H = False
    for i in range(num_coupling):   
        NP = NetProp(coupling_train,connectivity, hierarchical = H )
...
def get_D(W,A,signed = False):
    D = []
    Din = np.zeros(len(W))
    Dout=np.zeros(len(W))
    Dboth=np.zeros(len(W))
    if signed:
        for i in range(len(W)):
            Din[i] = sum(abs(W[i,:]))
            Dout[i] = sum(abs(W[:,i]))
            if A[i,i]>0: #need to adjust this
                Dboth[i] = abs(Din[i])+abs(Dout[i])-1
            else:
                Dboth[i] = abs(Din[i])+abs(Dout[i])
    else:
...
#    A = np.where(W>0, 1,0)
#    return A
def get_L(W, signed = False, normalized = False):
    A = calc_adj(W)
    D = get_D(W,A,signed)
    ...
    if normalized:
        Lin = D[0]**(-0.5)*Lin*D[0]**(-0.5)
        Lout = D[1]**(-0.5)*Lout*D[1]**(-0.5)
        Lboth = D[2]**(-0.5)*Lboth*D[2]**(-0.5)
    
    return([Lin,Lout,Lboth])

#definition of signed laplacian: chapter 4.1 (pages 162-165) in https://www.cis.upenn.edu/~cis515/cis515-15-spectral-clust-chap4.pdf
