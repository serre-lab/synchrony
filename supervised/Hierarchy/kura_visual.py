...
class displayer(object):
            ...
            for i in range(self.phases.shape[1]):
                for num in range(group_num):
                    if self.masks[0][num][i]:
                        ...
                        if annot_bool[num]:
                            plt.text(self.phases.shape[0] + 0.01, colors[num] * 2 * np.pi, 'group{}'.format(num),
    ...
    def set_phases(self, phases=None):
        if phases is not None:
            self.phases = np.squeeze(np.array(phases, dtype=np.float32))
            if len(self.phases.shape) < 2:
                self.phases = np.expand_dims(self.phases, axis=0)
        else:
            self.phases = None
        self.phases = (self.phases % (2 * np.pi) + 2 * np.pi) % (2 * np.pi)
        return True
    ...
    def last_phase(self, nrows, ncols, input, save_name, mask):
        fig, axes = plt.subplots(1, 3, figsize=(6, 4))
        axes.reshape(-1)
        axes[0].imshow(np.reshape(input, (nrows, ncols)), cmap='gray')
        axes[1].imshow(np.reshape(self.phases[-1], (nrows, ncols)), cmap='hsv')
        axes[2].imshow(np.reshape(mask, (nrows, ncols)), cmap='gray')
        axes[0].axis('off')
        axes[0].title.set_text('input')
        axes[1].axis('off')
        axes[1].title.set_text('last step')
        axes[2].axis('off')
