...
test_data_num = 100
batch_size = 20
show_every = 1 # show after every several batch
episodes = 15
record_step = 15
data_path = '/media/data_cifs/yuwei/osci_save/data/1000-types-mnist/multi-mnist10-0.1-sd0.5/3/valid/0'
...
save_path = '/media/data_cifs/yuwei/osci_save/results/multi_mnist_v56'
...
for i in tqdm(range(test_order.shape[0])):
    ...
    print(coupling[0][:-num_global_control, -num_global_control:])
    dads
    ...
    if (i % show_every) == 0:
        display(displayer, phase_list, images, masks, coupling, img_side, group_size,
                test_save_path, str(int(i / show_every)), rp_field)

if num_global_control == 0:
    regular_history = None

file = open(test_save_path + "/perform.txt", "w")
L = ['{} data test performance:{}\n'.format(test_data_num, loss_history/test_order.shape[0]),
     'Basset regularizer:{}'.format(regular_history/test_order.shape[0])]
file.writelines(L)
file.close()
