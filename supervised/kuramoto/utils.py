...
def lc_connectivity(img_side, num_cn, critic_dist):
    connectivity = np.zeros((img_side ** 2, num_cn))
    for i in tqdm(range(img_side ** 2)):
        count = 0
        for j in range(img_side ** 2):
            ...
            if (dist < critic_dist) and (dist > 0):
                connectivity[i, count] = j
                count += 1
        connectivity[i, count:] = i
    return connectivity


def generate_connectivity(img_side, num_cn, critic_dist, mode):
    if mode == 'local':
        return lc_connectivity(img_side, num_cn, critic_dist)
    elif mode == 'small_world':
        return sw_connectivity(img_side, num_cn, critic_dist)
    else:
        raise ValueError('Connectivity mode not exists')
