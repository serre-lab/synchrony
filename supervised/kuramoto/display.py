    ...
    def __init__(self, segments, interactive=False, phases=None, freqs=None, masks=None):
        ...
        if id(phases) != id(None) & id(freqs) != id(None):
            ...
            if masks is not None:
                self.fps = losses.fpt_btw_groups_np(self.phases, masks)
                self.inps = losses.inp_btw_groups_np(self.phases, masks)
                # Originally this ^^ said `np2`
                self.chs = losses.coh_btw_groups_np(self.phases, masks)
                self.abs = losses.abs_angle_diffs_np(self.phases)
            else:
                self.fps = losses.frame_pt_np(self.phases)
                self.inps = losses.inp_np(self.phases)
                self.chs = losses.coherence_np(self.phases)
                self.abs = losses.abs_angle_diffs_np(self.phases)
    ...
    def compute_properties(self):
        if self.phases is not None:
            if self.masks is None:
                ...
                self.fps = losses.fpt_btw_groups_np(self.phases, self.masks)
                self.inps = losses.inp_btw_groups_np(self.phases, self.masks)
                # Originally this ^^ said `np2`
                self.chs = losses.coh_btw_groups_np(self.phases, self.masks)
                self.abs = losses.abs_angle_diffs_np(self.phases)
