...
from MattLaplacian import Laplacian
import glob
"""
DataParallel
...
with torch.no_grad():
    for step, (test_data, _) in tqdm(enumerate(testing_loader)):
        ...
        coupling = coupling_test.cpu().numpy()
        for b in range(args.batch_size) :
            coupling_img = coupling[b]
            lp = Laplacian(coupling_img, weighted=True)
            val, vec = lp.get_eigen(signed=True, normalized=True)
            if labels[b][0] == 0:
                eig_0.append(val)
            else:
                eig_1.append(val)
        if step > num_batches:
            break
seg_2_eig = np.array(eig_0).reshape(-1)
seg_3_eig = np.array(eig_1).reshape(-1)

np.save('/media/data_cifs/matt/seg_2_eig_untrained.npy', seg_2_eig)
np.save('/media/data_cifs/matt/seg_3_eig_untrained.npy', seg_3_eig)
