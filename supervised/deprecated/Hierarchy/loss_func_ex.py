...
def matt_loss_torch(phase, mask, eta=.5):
    ...
    desynch_mask = ((~ und_mask).unsqueeze(1) * (~ und_mask).unsqueeze(2)).float()
...
def fpt_btw_groups_torch(phase, mask):
    ...
    diag_mat = (1 - torch.eye(sin_mat.shape[1], sin_mat.shape[1])).unsqueeze(0).to(device)
...
def inp_btw_groups_np(phase, mask):
    print("This is function will be deprecated, change to inp_btw_groups_np2")
    """
    deprecating
    :param phase: 
    :param mask: 
...
def inp_btw_groups_np2(phase, mask):
    groups_size = np.sum(mask, axis=2)
    groups_size_mat = np.matmul(np.expand_dims(groups_size, 2),
                                np.expand_dims(groups_size, 1))
    ...
    product = np.matmul(np.expand_dims(np.expand_dims(masked_sin, axis=2), axis=4),
        ...
        np.matmul(np.expand_dims(np.expand_dims(masked_cos, axis=2), axis=4),
                  np.expand_dims(np.expand_dims(masked_cos, axis=1), axis=3))
    # print(product.sum(4).sum(3))
    product = product / (np.expand_dims(np.expand_dims(groups_size_mat, axis=3), axis=4) + 1e-6)
    diag_mat = np.expand_dims(1 - np.eye(groups_size_mat.shape[1], groups_size_mat.shape[1]), axis=0)
    product = product.sum(4).sum(3) * diag_mat
    product = product.sum(2).sum(1) / np.abs(np.sign(product)).sum(2).sum(1)

    # final = np.sum(product, axis=2, keepdims=True) -\
            # (np.expand_dims(in_groups_product, axis=2) + 1e-6)
    # final = nan2zero_np(final)
    # print(final.sum(3).sum(2))
    #return np.round(np.exp(np.squeeze(product, axis=2).sum(3).sum(2).sum(1)\
           #/ 2 / [*torch.combinations(torch.arange(osci_num)).shape][0]), 5)
    # return np.round(np.squeeze(final, axis=2).sum(3).sum(2).mean(1), 5)
    return np.round(product, 5)


def exinp_btw_groups_torch(phase, mask):
    print("This is function will be deprecated, change to exinp_btw_groups_torch2")
    """
    deprecating
    :param phase: 
    :param mask: 
...
def exinp_btw_groups_torch2(phase, mask, device):
    phase = phase.to(device)
    mask = mask.to(device)
    groups_size = torch.sum(mask, dim=2)
    groups_size_mat = torch.matmul(groups_size.unsqueeze(2),
...
def exinp_in_groups_torch(phase, mask, device):
    phase = phase.to(device)
    mask = mask.to(device)
    groups_size = torch.sum(mask, dim=2)
    groups_size_mat = torch.matmul(groups_size.unsqueeze(2),
    ...
    product = (torch.matmul(masked_sin.unsqueeze(2).unsqueeze(4),
               ...
               torch.matmul(masked_cos.unsqueeze(2).unsqueeze(4),
                            masked_cos.unsqueeze(1).unsqueeze(3)))
    diag_mat = torch.eye(groups_size_mat.shape[1], groups_size_mat.shape[1]).unsqueeze(0).to(device)
    product = 1 - product.sum(4).sum(3) / (groups_size_mat + 1e-6)
    product = torch.exp(product) * diag_mat
    product = product.sum(2).sum(1) / torch.abs(torch.sign(product)).sum(2).sum(1)
    return product


def inp_btw_groups_torch(phase, mask):
    phase = phase.to(device)
    mask = mask.to(device)
    groups_size = torch.sum(mask, dim=2)
    groups_size_mat = torch.matmul(groups_size.unsqueeze(2),
...
def exinp_integrate_torch2(phase, mask, device):
    ...
    masked_ph = mask * phase.unsqueeze(1)

    product = torch.cos(masked_ph.unsqueeze(2).unsqueeze(4) - \
                        masked_ph.unsqueeze(1).unsqueeze(3))
    diag_mat = (1 - torch.eye(groups_size_mat.shape[1], groups_size_mat.shape[1])).unsqueeze(0).to(device)
    product_ = product.sum(4).sum(3) / (groups_size_mat + 1e-8)
    product_1 = torch.exp(product_) * \
                torch.where(groups_size_mat == 0,
...
def exinp_integrate_torch3(phase, mask, device):
    # integrate the calculation of both losses
    # do not avg over batch
    phase = phase.to(device)
    groups_size = torch.sum(mask, dim=2)
...
def coupling_regularizer(coupling, mask, device):
    ...
    return \
        (mask.sum(1, keepdim=True) * torch.bmm(torch.transpose(prod ** 2, 1, 2), mask) -
         torch.bmm(torch.transpose(prod, 1, 2), mask) ** 2).squeeze(1).sum(1) * 0.5 / ((mask.sum(1) - 1) * mask.sum(1)).sum(1)
...
def abs_angle_diffs_torch(phase):
    diffs = (phase.unsqueeze(1) - phase.unsqueeze(2)).to(device)
...
def nan2zero_np(tensor):
    tensor[np.isnan(tensor)] = 0
    return tensor


def nan2zero_torch(tensor):
