...
class Kuramoto(object):
    ...
    def phase_init(self, initialization='random', **kwargs):
        if initialization == 'random':
            self.phase_0 = lambda: 2 * np.pi * torch.rand((self.batch_size, self.N + self.gN)).to(self.device).float()
        elif initialization == 'fixed':
            self.current_phase = 2 * np.pi * torch.rand((1, self.N + self.gN)).repeat(self.batch_size, 1).to(
                self.device).float()
            self.phase_0 = lambda: self.current_phase
        elif initialization == 'gaussian':
            self.phase_0 = lambda: torch.normal(0., .1, (self.batch_size, self.N + self.gN)).to(self.device).float()
        elif initialization == 'categorical':
            self.phase_0 = lambda: torch.randint(0, 4, (self.batch_size, self.N + self.gN)).to(
                self.device).float() * 2 * np.pi / 4.
        elif initialization == 'random_walk':
            self.current_phase = 2 * np.pi * torch.rand((self.batch_size, self.N + self.gN)).to(self.device).float()
            self.gamma = kwargs['walk_step']

            def walk():
                self.current_phase += self.gamma * 2 * np.pi * torch.rand((self.batch_size, self.N + self.gN)).to(
                    self.device).float()
    ...
    def frequency_init(self, initialization='zero'):
        if initialization == 'gaussian':
            self.omega = lambda x: 2 * np.pi * torch.normal((self.batch_size, self.N + self.gN)).to(self.device)
    ...
    def _update2(self, coupling, omega):
        diffs = self.phase.unsqueeze(1) - self.phase.unsqueeze(2)
        self.delta = self.eps * (
                    torch.sum(coupling * torch.sin(diffs).gather(2, self.connectivity), dim=2) / coupling.shape[2])
        self.phase = self.phase + self.delta + omega
        return self.phase
        ...
        self.delta = self.eps * \
                     (torch.bmm(coupling, torch.sin(self.phase).unsqueeze(2).float()).squeeze(2) * torch.cos(
                      ...
                      torch.bmm(coupling, torch.cos(self.phase).unsqueeze(2).float()).squeeze(2) * torch.sin(
                                 self.phase)) / self.connectivity0.shape[-1]
        self.phase = self.phase + self.delta + omega
        return self.phase
        ...
        if hierarchical:
            local_coupling = torch.zeros(coupling[0].shape[0],
                                         ...
                                         self.phase.shape[1]).to(self.device).scatter_(dim=2,
                                                                                       index=self.connectivity[0].unsqueeze(0).repeat(coupling[0].shape[0], 1, 1),
                                                                                       src=coupling[0])
            global_coupling = torch.zeros(coupling[1].shape[0],
                                          coupling[1].shape[1],
                                          self.phase.shape[1]).to(self.device).scatter_(dim=2,
                                                                                        index=self.connectivity[1].unsqueeze(0).repeat(coupling[1].shape[0], 1, 1),
                                                                                        src=coupling[1])
            ...
            coupling = torch.zeros(coupling.shape[0], coupling.shape[1],
                                   coupling.shape[1]).to(self.device).scatter_(dim=2, index=self.connectivity.unsqueeze(0).repeat(coupling.shape[0], 1, 1),
                                                                               src=coupling)
