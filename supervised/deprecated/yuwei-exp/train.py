...
for epoch in range(args.train_epochs):
    ...
    for step, (train_data, _) in tqdm(enumerate(training_loader)):
        ...
        mask = torch.tensor(train_data[:, 1:, ...]).reshape(-1, args.segments, args.img_side * args.img_side).to(
            args.device).float()
        ...
        if step % args.show_every == 0:
            display(displayer, phase_list_train, batch, mask, coupling_train, omega_train, args.img_side, args.segments,
                    save_dir,
    ...
    for step, (test_data, _) in tqdm(enumerate(testing_loader)):
        ...
        if step % args.show_every == 0:
            ...
            coupling_test_show = \
                torch.zeros(1, args.img_side ** 2, args.img_side ** 2).to('cpu').scatter_(dim=2,
                                                                                          index=connectivity.cpu(),
                                                                                          src=coupling_test[
                                                                                              test_ind].cpu().unsqueeze(
                                                                                              0)).data.numpy()[0]
            ...
            show(displayer, test_phase_list, test_image, test_mask, coupling_test_show, omega_test_show, save_dir,
                 'test{}_{}'.format(epoch, step), args.segments, args.img_side)
