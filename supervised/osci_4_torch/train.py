...
class TrainPiprline(object):
    def __init__(self,
        ...
        if self.img_side > 2:
            test_data = generate_test_img(n=self.n, img_side=self.img_side, num=self.num)
            self.test_img = self.convert2oned(test_data[0], test=True)

            self.test_mask = torch.tensor(list(map(self._mask_pro, test_data[1]))).float()
        else:
    ...
    def opt(self):
        for t in range(self.steps):
            ...
            if t % self.report == 0:
                ...
                test_map = self.network(self.test_img)
                print('map', test_map)
                self.maps.append(test_map.data.numpy())
                #print(final_phase)
