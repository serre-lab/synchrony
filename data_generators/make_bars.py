...
def generate_bars(num_imgs, 
	...
	rand_locs = [rand_v_locs, rand_h_locs]
        ...
        cplx_img = np.concatenate((np.expand_dims(np.cos(img),0), np.expand_dims(np.sin(img),0)),axis=0) 
        np.save(os.path.join(save_dir,'img_%04d.npy'.format(num_bars) % n), cplx_img)

if __name__=='__main__':
    bgrnd_phase = True
    for nb in [1,2,3,4,5,6]:
        save_dir = os.path.join(os.path.expanduser('~'), 'data', 'synch_data','PHASE_BARS', str(nb), '0')
